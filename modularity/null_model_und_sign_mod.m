function [W0,R] = null_model_und_sign_mod(W,bin_swaps,wei_freq)
...
if nnz(Ap)<(n*(n-1))                                        %if Ap is not full
    W_r  = randmio_und_signed_mod(W,bin_swaps);
    Ap_r = W_r>0;
    An_r = W_r<0;
else
    Ap_r = Ap;
...
for s=[1 -1]
    switch s                                                %switch sign (positive/negative)
        case 1
            S=sum(W_no_NaN.*Ap,2);                          %positive strength
            Wv=sort(W_no_NaN(triu(Ap)));                    %sorted weights vector
            [I,J]=find(triu(Ap_r));                         %weights indices
            Lij=n*(J-1)+I;                                  %linear weights indices
        case -1
            S=sum(-W_no_NaN.*An,2);                         %negative strength
            Wv=sort(-W_no_NaN(triu(An)));                   %sorted weights vector
...
mask = tril(true(n));
W0(mask) = NaN;
rpos=corrcoef(sum( W_no_NaN.*(W>0)),sum( W0.*(W0>0)));
rneg=corrcoef(sum(-W_no_NaN.*(W<0)),sum(-W0.*(W0<0)));
R=[rpos(2) rneg(2)];
