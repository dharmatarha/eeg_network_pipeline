...
for freqBandIndex = 1 : numberOfFrequencyBands
    ...
    for connMetricIndex = 1 : numberOfConnMetrics
        ...
        load([dirName, '/', fileName], 'maskedConn');
        connectivityTensor_thr = maskedConn;
        epochLevelConnVectors = nan(subjectNo, epochNo, roiNo*(roiNo-1)/2);
        for subjectIndex = 1 : subjectNo
            for epochIndex = 1 : epochNo
                epochLevelConnMatrix = squeeze(connectivityTensor_thr(subjectIndex, epochIndex, :, :));
    ...
    title(['No avg. corr. ', freqBandString, ' band, thr']);    
    colormap('jet');
    caxis([-0.8 1]);
    colorbar;
    set(gca, 'FontSize', 14);
