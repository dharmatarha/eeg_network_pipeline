function [permRes, withinCondAnova, connSim] = cmpFullConn_edgeRm(connData, varargin)
...
% connectivity matrix.
%
% Unlike cmpFullConn_edge.m, the current script evaluates edge 
% contributions by deleting edges one-by-one and calculating the within- 
% vs. across-condition simlarity results for each "reduced" network.
% In comparison, cmpFullConn_edge.m evaluates edge contributions by testing
% for within- vs across-condition similarity effects for single edges by
% themselves (when the similarity mteric allows it: correlation and
% difference matrix norm). 
% The approach of the current function - in theory - allows us to evaluate
% edge contributions using any similarity metric.
%
% For within- vs across-condition similarity effects, the function compares 
% connectivity (edge) values across different groups of 
...
% matrix used for edge IDs - this method is preferred so that the results
% can be compared with other methods + easily turned back into the format
% of the connectivity matrix
edgeNumbers = nan(roiNo);
edgeNumbers(triu(true(roiNo), 1)) = 1:edgeNo;
...
for edgeIdx = 1:edgeNo
    ...
    % copy of connectivity data used for edge removal
    connEdgeRm = connData;
    % set current edge to zero in connectivity data
    [iRow, iCol] = find(edgeNumbers == edgeIdx);
    connEdgeRm(iRow, iCol, :) = 0;
    ...
    if strcmp(metric, 'corr')
        ...
        dataLin = linearizeTrius(connEdgeRm, 1);  
        % get all column-pairwise correlations
        edgeConnSim = corr(dataLin, dataLin);
        % keep the upper triangle, inlcuding main diagonal, set the rest to NaN
        edgeConnSim(tril(true(epochNo*condNo), -1)) = nan;
        % aggregate edge-level results into connSim
        connSim(edgeIdx, :, :) = edgeConnSim;

    % frobenius norm of difference matrix ('eucl') or DeltaCon
    elseif ismember(metric, {'eucl', 'deltaCon'})
        % preallocate results matrix
        edgeConnSim = nan(epochNo*condNo);
        % loops through epochs
        for epochOne = 1:epochNo*condNo
            % create symmetric adjacency matrices with zeros at diagonal
            epochOneData = triu(connEdgeRm(:, :, epochOne), 1) + triu(connEdgeRm(:, :, epochOne), 1)';
            for epochTwo = 1:epochNo*condNo
                if epochOne <= epochTwo  % only for upper triangle of pairings

                    % create symmetric adjacency matrices with zeros at diagonal
                    epochTwoData = triu(connEdgeRm(:, :, epochTwo), 1) + triu(connEdgeRm(:, :, epochTwo), 1)';

                    % calculation depending on metric
                    switch metric
                        case 'eucl'
                            edgeConnSim(epochOne, epochTwo) = norm(epochOneData-epochTwoData, 'fro');
                        case 'deltaCon'
                            edgeConnSim(epochOne, epochTwo) = deltaCon(epochOneData, epochTwoData, false);  % verbosity of deltaCon is set to false
                    end
